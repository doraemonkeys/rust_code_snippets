...
fn main() {
    ...
    // 打印一条警告信息
    println!("cargo:warning=构建脚本正在执行...");

    // 构建脚本的任何其他标准输出（比如通过println!宏输出的信息）并不会直接显示在终端上，
    // 而是被Cargo捕获。只有当构建脚本执行失败（即退出码非零）时，
    // 这些输出才会被Cargo显示出来，以帮助诊断问题。
    // 如果你希望在构建过程中看到构建脚本的输出（不仅仅是在出错时），
    // 你可以在运行cargo build命令时增加--verbose（或-v）参数。
    // 使用这个参数，Cargo会输出更多的信息，包括构建脚本的标准输出。例如：
}
